...
"""
    ...
    assert ret == {
        finder.Plugin(
            "flake8-foo",
            "1.2.3",
            importlib.metadata.EntryPoint("Q", "flake8_foo:Plugin", "flake8.extension"),
        ),
        finder.Plugin(
            "pycodestyle",
            "9000.2.0",
...
def test_find_plugins(
    ...
    assert ret == [
        ...
        finder.Plugin(
            "local",
            "local",
            importlib.metadata.EntryPoint("Y", "mod2:attr", "flake8.extension"),
        ),
        finder.Plugin(
            "local",
            "local",
...
def test_classify_plugins_errors_on_incorrect_checker_name():
    ...
    (msg,) = excinfo.value.args
    assert msg == (
        "plugin code for `local[INVALID]` " "does not match ^[A-Z]{1,3}[0-9]{0,3}$"
    )
